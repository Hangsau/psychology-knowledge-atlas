...
`anzsrc-2020-for-indigenous-studies-groups` 完整登錄 ANZSRC Division 45 的 20 個 group-level codes，保留 Aboriginal and Torres Strait Islander、Māori、Pacific Peoples，以及 Indigenous data／methodologies／global studies 的分立軸線。此 contextual inventory 不把 Indigenous psychology 縮減成健康或社會單一面向；納入項使用 `context_domain`，不冒充心理學 `subfield`。4501–4512 已裁決，Māori records 以 te reo Māori 為 canonical name、英文標籤為 alias；Pacific Peoples 與 global methodology 共 7 個實質候選待裁決，`4599 Other Indigenous studies` residual 排除，因此 `complete:true`、`resolved:false`。Pacific Peoples 的 `4517 society and community`／`4518 sciences` 依 2025-10-24 修正版 ABS data cube，而非舊 ARC guide 排列。
