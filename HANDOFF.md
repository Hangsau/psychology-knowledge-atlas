...
next_gate: produce the first VERIFIED RESEARCH-tier claim (not popular framing). Prioritize open replication / meta-analysis full text: search Many Labs, the Reproducibility Project, Registered Reports, PsyArXiv, or an open-access journal for a psychology finding whose primary evidence is genuinely readable this session. Best pilot candidates: an open replication of the misattribution-of-arousal bridge effect, or an open meta-analysis bearing on Dunning-Kruger. Read the actual full text, then build a research (finding) claim end-to-end: source access_status readable, evidence_level fulltext_direct + verbatim short_quote + specific locator, claim status verified + publishable:true. For contested effects, an "effect is real" claim still needs meta-analysis/replication evidence. Keep atlas-level evidence_release false. If no full text can actually be read, STOP and register it in the honest citation/queue layer — do not fake a quote or a read.
```
...
`evidence_release: false`（atlas 層仍未發布）。已有第一筆通過閘門的 `publishable:true` claim（`c-dk-popular`，達克效應的通俗說法，非「效應為真」主張），其餘 claim 落在誠實引用／待辦層或候選層。個別 claim 可 publishable 不等於整個 atlas 進入 evidence_release。
