...
`structuralism` 已完成 S1-S7 退出門檻，並額外完成一個不改變 gate 順序的薄型 P4 reader preview。當前 bounded target 是 `psychoanalysis`；其七題已固定，S1-S3 已完成並有可重生的繁中 reader preview。完整 P4 views 仍待四個 pilots 後進行。
